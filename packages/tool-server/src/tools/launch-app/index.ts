...
Returns { launched, bundleId, note? }. Fails if the app is not installed on the target device (iOS / Android). On a physical iPhone this registers the app every other tool acts on; com.apple.springboard and com.apple.Spotlight register without launching. note warns when runner signing is not ready. On Android a launch that overruns the platform's wait window is confirmed by checking the app's process instead of failing, and note then says the app may not be interactive yet.

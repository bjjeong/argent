...
On a physical Android device, Argent starts a screen-sharing agent on the device over adb. Argent stops this agent when the session ends or when the connection to the device stops. The agent can stay active if the host process stops abruptly:
...
To stop the agent manually, run `adb -s <serial> shell pkill -f 'screensharing[.]Main'`. This command also stops the device mirror of Android Studio.

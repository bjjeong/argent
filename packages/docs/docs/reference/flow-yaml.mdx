...
- The `expected:`, `actual:` and `hint:` lines of that step that apply to the failure.
...
When a single flow run fails its steps, Argent prints the first step that failed or errored again above the last line. Under that step, Argent prints its reason and its `expected:`, `actual:` and `hint:` lines again.
